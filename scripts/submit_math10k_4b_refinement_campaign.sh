...
# Submit one portable two-A100 allocation for the focused 4B PRISM/SlaClip
# C=1--2 refinement and fresh-seed paired confirmation campaign.  This wrapper performs
...
usage() {
  cat <<'EOF'
Usage:
  scripts/submit_math10k_4b_refinement_campaign.sh --test-only
  scripts/submit_math10k_4b_refinement_campaign.sh --submit
  scripts/submit_math10k_4b_refinement_campaign.sh --resume-submit
...
27-candidate local screening, five-seed selection confirmation, and five
fresh-seed paired full-data evaluations against fixed C=2.
...
CAMPAIGN_ID="${PRISM_CAMPAIGN_ID:-math10k-4b-c1-c2-refinement-${SHORT_SHA}-v1}"
...
GPU_GRES="${PRISM_GPU_GRES:-gpu:a100:2}"
...
if [[ "${GPU_GRES}" != "gpu:a100:2" ]]; then
  echo "error: this reviewed protocol requires PRISM_GPU_GRES=gpu:a100:2" >&2
  exit 2
fi
...
for tracked_file in \
  ...
  slurm/math10k_4b_refinement_campaign.sbatch; do
...
WORKER="${STAGED_REPO_ROOT}/slurm/math10k_4b_refinement_campaign.sbatch"
if [[ ! -f "${WORKER}" ]]; then
...
echo "resources=account:${ACCOUNT},qos:${QOS},partition:${PARTITION},2xA100,16cpu,${HOST_MEMORY},${WALLTIME},exclude:${EXCLUDE_NODES:-none}"
echo "protocol=27_candidate_c1_c2_local_grid_then_five_seed_selection_then_five_fresh_paired_final"
